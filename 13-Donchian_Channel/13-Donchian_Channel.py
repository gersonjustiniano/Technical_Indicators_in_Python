...
class Donchian_channel:
    def __init__(self):
        self.n=100
        self.fig=plt.figure()
        self.title='Donchian Channel'
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.donchian_dict(14,var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.donchian_pandas(14,var)
    ...
    def plot_data(self,var):
        ...
        ax1.plot(var['time'],var['upper_band'],c='dimgray',lw=1)
        ax1.plot(var['time'],var['lower_band'],c='dimgray',lw=1)
        ax1.plot(var['time'],var['median_price'],c='dimgray',lw=1)
    
        plt.show()

obj=Donchian_channel()
