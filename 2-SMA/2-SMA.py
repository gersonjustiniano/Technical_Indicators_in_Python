...
class SMA:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                n_list=[20,50]
                for n in n_list:
                    self.indi.sma_dict(n,'close',var)
        ...
        if self.calc_type=='pandas':
            ...
            n_list=[20,50]
            for n in n_list:
                df[f'sma{n}']=df.apply(lambda row: self.indi.sma_pandas(n,'close',(row,var)),axis=1)
                var[f'sma{n}']=df[f'sma{n}'].to_list()
    ...
    def plot_data(self,var):
        ...
        #PLOT SMA:
        sma_keys=[k for k in var.keys() if 'sma' in k]
        if len(sma_keys)>=1:
            for sma in sma_keys:
                ax.plot(var['time'],var[sma],lw=1.5,label=sma)
            ax.legend()
