...
class Heikin_Ashi:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.heikin_dict(var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.heikin_pandas(var)
    ...
    def plot_data(self,var):
        ...
        for i in range(len(var['time'])):
            if var['h_open'][i]>var['h_close'][i]:
                ccolor='r'
            elif var['h_open'][i]<var['h_close'][i]:
                ccolor='g'
            else:
                ccolor='gray'
            ax2.plot([var['time'][i]]*2,[var['h_open'][i],var['close'][i]],c=ccolor,lw=3)
            ax2.plot([var['time'][i]]*2,[var['h_high'][i],var['h_low'][i]],c=ccolor,lw=0.5)
