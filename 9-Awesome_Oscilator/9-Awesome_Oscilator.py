...
class Awesome:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.awesome_dict(5,34,var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.awesome_pandas(5,34,var)
    ...
    def plot_data(self,var):
        ...
        for i in range(len(var['ao'])):
            if i==len(var['ao'])-1:
                break
            if var['ao'][i+1]>var['ao'][i]:
                ccolor='green'
            elif var['ao'][i+1]<var['ao'][i]:
                ccolor='red'
            else:
                ccolor='gray'
            ax2.bar(var['time'][i],var['ao'][i],color=ccolor)
