...
class HMA:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.hma_dict(20,var)

            self.print_data(var)
        
        if self.calc_type=='pandas':
            self.indi.hma_pandas(20,var)
    ...
    def plot_data(self,var):
        ...
        for i in range(len(var['time'])):
            if i>=1:
                if var['hma20'][i]>var['hma20'][i-1]:
                    ccolor='g'
                elif var['hma20'][i]<var['hma20'][i-1]:
                    ccolor='r'
                else:
                    ccolor='gray'
                ax1.plot([var['time'][i-1],var['time'][i]],[var['hma20'][i-1],var['hma20'][i]],c=ccolor,lw=2)
