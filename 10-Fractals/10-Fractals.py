...
class Fractals:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.fractals_dict(2,var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.fractals_pandas(2,var)
    ...
    def plot_data(self,var):
        ...
        #'''
        #plot fractals:
        up_time=[t[0] for t in var['up']]
        up_price=[p[1] for p in var['up']]
        down_time=[t[0] for t in var['down']]
        down_price=[p[1] for p in var['down']]
        ax1.scatter(up_time,up_price,c='black',marker='^',s=10)
        ax1.scatter(down_time,down_price,c='black',marker='v',s=10)
        #'''
