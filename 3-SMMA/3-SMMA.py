...
class SMMA:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                n_list=[20,50]
                for n in n_list:
                    self.indi.smma_dict(n,'close',var)
        ...
        if self.calc_type=='pandas':
            n_list=[20,50]
            for n in n_list:
                var[f'smma{n}']=self.indi.smma_pandas(n,'close',var)
    ...
    def plot_data(self,var):
        ...
        #PLOT SMMA:
        smma_keys=[k for k in var.keys() if 'smma' in k]
        if len(smma_keys)>=1:
            for smma in smma_keys:
                ax.plot(var['time'],var[smma],lw=1.5,label=smma)
            ax.legend()
