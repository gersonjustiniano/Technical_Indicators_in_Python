...
class ZLEMA:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.zlema_dict(20,var)

            self.print_data(var)
        
        if self.calc_type=='pandas':
            self.indi.zlema_pandas(20,var)
    ...
    def plot_data(self,var):
        ...
        for i in range(len(var['time'])):
            if i>=1:
                if var['zlema20'][i]>var['zlema20'][i-1]:
                    ccolor='g'
                elif var['zlema20'][i]<var['zlema20'][i-1]:
                    ccolor='r'
                else:
                    ccolor='gray'
                ax1.plot([var['time'][i-1],var['time'][i]],[var['zlema20'][i-1],var['zlema20'][i]],c=ccolor,lw=2)
