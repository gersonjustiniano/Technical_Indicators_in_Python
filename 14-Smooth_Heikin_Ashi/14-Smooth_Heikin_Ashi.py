...
class Smooth_Heikin_Ashi:
    def __init__(self):
        self.n=100
        self.fig=plt.figure()
        self.title='Smooth Heikin Ashi'
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.smooth_heikin_dict(20,var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.smooth_heikin_pandas(20,var)
    ...
    def plot_data(self,var):
        ...
        ax1=self.fig.add_subplot(ax[0:6,:])
        ...
        #plot smooth heikin ashi:
        for i in range(len(var['time'])):
            if var['sh_open'][i]>var['sh_close'][i]:
                ccolor='darkred'
            elif var['sh_open'][i]<var['sh_close'][i]:
                ccolor='darkgreen'
            else:
                ccolor='gray'
            ax1.plot([var['time'][i]]*2,[var['sh_open'][i],var['sh_close'][i]],c=ccolor,lw=3)
            ax1.plot([var['time'][i]]*2,[var['sh_high'][i],var['sh_low'][i]],c=ccolor,lw=0.5)
    
        plt.show()

obj=Smooth_Heikin_Ashi()
