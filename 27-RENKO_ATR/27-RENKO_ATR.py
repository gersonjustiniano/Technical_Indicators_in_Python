...
class RENKO_ATR:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.RenkoAtr_dict(14,var)

            self.print_data(var)
        
        if self.calc_type=='pandas':
            self.indi.RenkoAtr_pandas(14,var)
    ...
    def plot_data(self,var):
        ...
        ax1=self.fig.add_subplot(ax[0:3,:])
        ax2=self.fig.add_subplot(ax[3:6,:])
        ...
        ax2.set_xticks([])
        ax2.set_xlim(-1,fr+1)
        if len(var['renko_close']) > 0:
            for i in range(len(var['renko_close'])):
                if var['renko_open'][i]>var['renko_close'][i]:
                    ccolor='r'
                elif var['renko_open'][i]<var['renko_close'][i]:
                    ccolor='g'
                else:
                    ccolor='gray'
                ax2.plot([i+1]*2,[var['renko_open'][i],var['renko_close'][i]],c=ccolor,lw=3)
        else:
            print('no hay ladrillos')
