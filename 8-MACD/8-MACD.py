...
class MACD:
    ...
    def get_data(self,var):
        for i in range(self.n):
            ...
            if self.calc_type=='dict':
                self.indi.macd_dict(12,26,9,'close',var)

            self.print_data(var)

        if self.calc_type=='pandas':
            self.indi.macd_pandas(12,26,9,'close',var)
    ...
    def plot_data(self,var):
        ...
        ax2.set_xticks([])
        ax2.set_xlim(-1,101)
        ax2.axhline(y=0,lw=1,color='black')

        for i in range(len(var['macd_histogram'])):
            if i>0:
                if var['macd_histogram'][i]>var['macd_histogram'][i-1]:
                    ccolor='darkgreen'
                elif var['macd_histogram'][i]<var['macd_histogram'][i-1]:
                    ccolor='darkred'
                else:
                    ccolor='gray'
            ax2.bar(var['time'][i],var['macd_histogram'][i],color=ccolor)

        ax2.plot(var['time'],var['macd'],c='r',lw=1)
        ax2.plot(var['time'],var['signal_macd'],c='g',lw=1)
